...
class PPOAlgo(BaseAlgo):
    ...
    def __init__(self, envs, acmodel, device=None, num_frames_per_proc=None, discount=0.99, lr=0.0001, gae_lambda=0.95,
                 ...
                 max_num_train_seeds=-1,init_train_seed=0,
                 max_num_test_seeds=-1,init_test_seed=0,
                 envs_ood=None, env_ood_name=None, ood_method=None,
                 init_train_seed_ood=0, max_num_train_seeds_ood=-1,
                 init_test_seed_ood=0, max_num_test_seeds_ood=-1, ood_sampling_ratio=0.0,
                 intervene_after=-1, intervene_every=-1, adaptation_strategy="",
                 window_visible=False, self_supervised_model=None):
        num_frames_per_proc = num_frames_per_proc or 128

        super().__init__(envs, acmodel, device, num_frames_per_proc, discount, lr, gae_lambda, entropy_coef,
                         value_loss_coef, max_grad_norm, recurrence, preprocess_obss, reshape_reward,
                         separated_networks,env_name, num_actions, int_coef, normalize_int_rewards,
                         im_type,int_coef_type,use_episodic_counts,use_only_not_visited,total_num_frames,
                         reduced_im_networks,
        ...
        self.ssmodel = self_supervised_model
        ...
        if self.separated_actor_critic:
            ...
            if self.ssmodel is not None:
                # self-supervised
                conv_params = list(self.acmodel.image_conv.parameters())
                idm_params = list(self.ssmodel.parameters())
                self.optimizer_idm = torch.optim.Adam(conv_params + idm_params,
                                                    lr, eps=adam_eps)
    ...
    def update_ssl_with_idm(self, exps):

        # sb = exps[:]
        # print(sb)
        # print(type(sb))
        # from numpy to torch
        # obs = torch.from_numpy(sb.obs).to(self.device).float()
        # acs = torch.from_numpy(exps.acs[:-1]).to(self.device).long()

        # obtain embeddings (forward pass embedding)
        _dist, _obs_embedding, _value_ext = self.acmodel(obs=exps.obs, ood_method='pca')
        obs_embedding = _obs_embedding[:-1]
        next_obs_embedding = _obs_embedding[1:]
        # take corresponding actions
        actual_actions = exps.action[:-1].long()

        
        # forward pass through self-supervised model
        _, log_soft_pred_actions = self.ssmodel(state_embedding=obs_embedding,
                                    next_state_embedding=next_obs_embedding)
                
        # calculate the loss respect the ground truth
        ssl_loss = F.nll_loss(log_soft_pred_actions, 
                              target=actual_actions)

        # reset and execute backward optimization
        self.optimizer_idm.zero_grad()
        ssl_loss.backward()
        self.optimizer_idm.step()
        
        return {'ssl_loss': ssl_loss.item()}
