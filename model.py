...
from constants import CONV_METHODS, LOGITS_METHODS, IM_MOD_METHODS
...
class ACModelRIDE(nn.Module, torch_ac.RecurrentACModel):
    def __init__(self, obs_space, action_space, use_intcoefs=0, use_memory=False, use_text=False):
        ...
        # TODO: Added to enable the generation of explanations
        ...
        # self.conv2 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=(3,3), stride=(2,2), padding=1)
        # self.conv3 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=(3,3), stride=(2,2), padding=1)
        ...
        # Image goes from a 7x7x3 to a 1x1x32
        self.image_conv = nn.Sequential(
    ...
    def forward(self, obs, int_coefs=[], memory=[], ood_method=''):

        # TODO: La libreria de explicabilidad requiere que se tenga que recibir directamente el tensor de la imagen.
        #   De momento me voy a arreglar cambiando esto manualmente en funcion de lo que quiera usar, pero tendre que estandarizar.
        #   Para ello lo mejor será que cuando el propio agente llame al forward, se le pase el tensor de la imagen y no el objeto DictList

        # Estos sería si pasamos directamente la imagen -> x = obs.transpose(1, 3).transpose(2, 3)
        x = obs.image.transpose(1, 3).transpose(2, 3)
        x = self.image_conv(x)
        x = x.reshape(x.shape[0], -1)

        if ood_method in CONV_METHODS:
            activations = x  # deepcopy(x)
        ...
        # _logits = self.actor(embedding)
        # x = F.log_softmax(_logits, dim=1)
        # dist = Categorical(logits=x)

        x = self.actor(embedding)
        if ood_method in LOGITS_METHODS:
        ...
        if ood_method in IM_MOD_METHODS:  # No need for internal activations
            activations = 0
        ...
        if len(memory)>0:
            ...
            if ood_method:
                return dist, activations, value_ext
...
class InverseDynamicsNetwork(nn.Module):
    def __init__(self, num_actions, input_size = 64):
        super().__init__()
    ...
    def forward(self, state_embedding, next_state_embedding):
        ...
        x = self.id_out(self.inverse_dynamics(inputs))
        
        action_logits = F.log_softmax(x, dim=1)
        dist = Categorical(logits=action_logits)
        return dist, action_logits